...
int main(int argc, char **argv){
    ...
    while(optind < argc){
    	if ((opt = getopt(argc, argv, opts)) != -1){
	    	...
	    	num_el = atoi(argv[optind]);
	    	optind+=1;
	    	num_ar = atoi(argv[optind]);
	    	optind+=1;
	    }
    }
	...
	if (strcmp(alg,"qsort") == 0){
		for(int i = 0; i<num_ar; i++){
			clock_t start = clock();
			qsort(array[i], num_el, sizeof(Abonent), (int (*)(const void *, const void *)) fp);
			clock_t end = clock();
		...
		for(int i = 0; i<num_ar; i++){
			clock_t start = clock();
			gnome_sort(array[i], num_el, sizeof(Abonent), (int (*)(const void *, const void *)) fp);
			clock_t end = clock();
			double sort_time = (double)(end-start)/CLOCKS_PER_SEC;
			total_time += sort_time;
		}
		...
		for(int i = 0; i<num_ar; i++){
			clock_t start = clock();
			shell_sort(array[i], num_el, sizeof(Abonent), (int (*)(const void *, const void *)) fp);
			clock_t end = clock();
			double sort_time = (double)(end-start)/CLOCKS_PER_SEC;
			total_time += sort_time;
		}
